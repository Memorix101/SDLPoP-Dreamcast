...
#ifdef DREAMCAST
#define access custom_access
#define F_OK 0 // Only checks whether the file exists
#define R_OK 4 // Checks read access
#define W_OK 2 // Checks write access
...
void init_scaling(void) {
	...
	if (texture_sharp == NULL) {
		texture_sharp = SDL_CreateTexture(renderer_, SDL_PIXELFORMAT_RGB888, SDL_TEXTUREACCESS_STREAMING, 320, 200);
	}
	if (scaling_type == 1) {
		...
		if (texture_fuzzy == NULL) {
			...
			texture_fuzzy = SDL_CreateTexture(renderer_, SDL_PIXELFORMAT_RGB888, access, 320*2, 200*2);
		...
		if (texture_blurry == NULL) {
			SDL_SetHint(SDL_HINT_RENDER_SCALE_QUALITY, "1");
			texture_blurry = SDL_CreateTexture(renderer_, SDL_PIXELFORMAT_RGB888, SDL_TEXTUREACCESS_STREAMING, 320, 200);
			SDL_SetHint(SDL_HINT_RENDER_SCALE_QUALITY, "0");
		}
...
void lcd_test() {
	...
	for(int i = 0; i < 4; i++){
    while(vmu_task == 0) {
        maple_device_t *addr = maple_enum_type(i, MAPLE_FUNC_LCD);
	...
	while (SDL_PollEvent(&event) == 1) { // while there are still events to be processed
		switch (event.type) {
...
#endif
#ifdef USE_MENU
				if (escape_key_suppressed && (scancode == SDL_SCANCODE_BACKSPACE || (enable_pause_menu && scancode == SDL_SCANCODE_ESCAPE))) {
			...
			case SDL_CONTROLLERBUTTONUP:
				switch (event.cbutton.button)
				{
					printf("up event.cbutton.button %d\n", event.cbutton.button);
			...
			case SDL_JOYAXISMOTION:
				...
				if (event.type == SDL_JOYBUTTONDOWN) {
					...
					if(event.jbutton.button == 2){ // dc a btn
